...
async function setClientCondition(data) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        if (clientCondition === "Random") {
            ...
            if (sessionStorage && sessionStorage.getItem("oldChat") && sessionStorage.getItem("oldChat").length && !(sessionStorage.getItem("couponAssigned") && (sessionStorage.getItem("couponAssigned") === "1"))) {
                ...
                if (hasCoupon.expired) {
                    ...
                    setTimeout(() => {
                        lexHelper("Thank you for checking out Inteliyo!", "Other", 0);
        ...
        // Screen Time
            ...
            // To get Seconds
            let timeAB = int(data[i].label.slice(-2));
...
console.log("heyeye123");
    ...
    await fetch("https://t6mcmorgdb.execute-api.us-east-1.amazonaws.com/ABTestActionSelection/getcampaigns?companyName=Reiker&campaignname=")
        .then(response => response.json())
...
function sendFirst(hello, flag, hasCoupon, t) {
    ...
    if (modal.style.display !== "block") {
        ...
        var PopUp = new Audio();
        PopUp.src = "https://inteliyo.net/wp-content/uploads/2020/06/PopUp-1.mp3";
        PopUp.play();
    }
        ...
        if (!hasCoupon.assigned) {
            ...
            const dataToPush = {
                ...
                body: JSON.stringify({
                    ...
                    "uDeviceType": capturedData[8] === "" ? "Unknown" : capturedData[8].toLowerCase(),
                    "uDeviceBrand": capturedData[9] === "" ? "Apple" : capturedData[9],
                      "product_Category": finalValues ? finalValues : "",
                    // "uLatitude": capturedData[12] === "" ? "0.0" : capturedData[12],
                    // "uLongitude": capturedData[13] === "" ? "0.0" : capturedData[13],
...
function getRegion(latitude, longitude, type) {
    // Debugger
...
if (document.title === "Checkout - Inteliyo" && sessionStorage && sessionStorage.getItem("userDiscount") && sessionStorage.getItem("userDiscount").length) {
...
async function updateCouponWithExpiry(id) {
    ...
    await fetch("https://inteliyo.net/wp-json/wc/v3/coupons/" + id.toString() + "?consumer_key=ck_e7114a238793b14fcfe814ca07869fe518e56675&consumer_secret=cs_b3e77e0443446ded464c05bb1bc83a5c46a4e342", updatedCoupon);
}
...
async function lexHelper(data, type, flag) {
    ...
    if (couponShown === "Hi!" || couponShown === "Hi! Glad to see you again :)" || couponShown === "Thank you for checking out Inteliyo!" || couponShown === "We've got a deal just for you!" || couponShown[0] === "W") {
        couponShown = "";
    }
    ...
    if (couponShown !== "") {
        ...
        if (!hasCoupon.assigned && !(sessionStorage && sessionStorage.getItem("couponAssigned") && sessionStorage.getItem("couponAssigned") === "1")) {
            ...
            if (!sessionStorage.getItem("freeShipping") || sessionStorage.getItem("freeShipping") !== "1") {
                // Making a New Coupon
                const newCoupon = {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                await fetch("https://inteliyo.net/wp-json/wc/v3/coupons?consumer_key=ck_e7114a238793b14fcfe814ca07869fe518e56675&consumer_secret=cs_b3e77e0443446ded464c05bb1bc83a5c46a4e342", newCoupon)
                    .then(response => response.json())
    ...
    // If only message is there, Render it by using tags
        ...
        if (couponShown !== "") {
            ...
            let revealAndCopy = document.createElement("button");
            revealAndCopy.innerHTML = "Reveal & Copy";
            revealAndCopy.setAttribute("id", "revealAndCopy");
            revealAndCopy.setAttribute("class", "botButton");
        ...
        if (modifiedInnerHTML !== "Hi!" && modifiedInnerHTML !== "Hi! Glad to see you again :)" && modifiedInnerHTML !== "Thank you for checking out Inteliyo!" && modifiedInnerHTML !== "We've got a deal just for you!" && modifiedInnerHTML[0] !== "W") {
            ...
            function timerIncrement() {
                ...
                if (idleTime > iaTime && nudgeRemover != "Hey there! Is there anything else we can help you with?" && nudgeRemover != "Alright! Thanks for letting me know. If you need my help, you can check out the FAQ section. Happy Shopping :)") {
                    ...
                    if (modal.style.display === "block") {
                        ...
                        remindUser.src = "https://inteliyo.net/wp-content/uploads/2020/06/PopUp-1.mp3";
                        remindUser.play();
                    }
